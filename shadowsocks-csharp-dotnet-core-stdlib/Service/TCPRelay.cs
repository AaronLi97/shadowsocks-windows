...
using Shadowsocks.Std.Encryption.AEAD;
...
using static Shadowsocks.Std.Encryption.EncryptorBase;
...
namespace Shadowsocks.Std.Service
{
    internal class TCPRelay : Listener.Service
    {
        ...
        public override bool Handle(byte[] firstPacket, int length, Socket socket, object state)
        {
            ...
            foreach (var tcpHandler in handlersToClose)
            {
                _logger.Debug("Closing timed out TCP connection.");
                tcpHandler.Close();
            }
    ...
    internal class TCPHandler
    {
        ...
        public void CreateRemote()
        {
            ...
            _logger.Debug($"_addrBufLength={_addrBufLength}");
            ...
            try
            {
                ...
                if (bytesRead > 1)
                {
                    ...
                    if (_firstPacket[0] != 5)
                    {
                        ...
                        _logger.Error("socks 5 protocol error");
                    }
            ...
            try
            {
                ...
                if (bytesRead >= 5)
                {
                    ...
                    switch (_command)
                    {
                        ...
                        default:
                            _logger.Debug("Unsupported CMD=" + _command);
                            Close();
                            break;
                    }
                }
                ...
                {
                    _logger.Debug("failed to recv data in Shadowsocks.Controller.TCPHandler.handshakeReceive2Callback()");
                    Close();
                }
            }
            catch (Exception e)
        ...
        private void ReadAddress(Action onSuccess)
        {
            ...
            switch (atyp)
            {
                ...
                default:
                    _logger.Debug($"Unsupported ATYP={atyp}");
                    Close();
                    break;
            }
        }
        ...
        private void OnAddressFullyRead(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (bytesRead >= bytesRemain)
                {
                    ...
                    {
                        _logger.Info($"connect to {dstAddr}:{dstPort}");
                    }
                ...
                {
                    _logger.Debug("failed to recv data in Shadowsocks.Controller.TCPHandler.OnAddressFullyRead()");
                    Close();
                }
            }
            catch (Exception e)
        ...
        private void ProxyConnectTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            _logger.Info($"Proxy {proxy.ProxyEndPoint} timed out");
            proxy.Close();
            Close();
        }
        ...
        private void ProxyConnectCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (_config.isVerboseLogging)
                {
                    ...
                    {
                        _logger.Info($"Socket connected to proxy {remote.ProxyEndPoint}");
                    }
                }
        ...
        private void DestConnectTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            _logger.Info($"{server.FriendlyName()} timed out");
        ...
        private void ConnectCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (_config.isVerboseLogging)
                {
                    _logger.Info($"Socket connected to ss server: {_server.FriendlyName()}");
                }
        ...
        private void StartPipe(AsyncSession session)
        {
            ...
            try
            {
                ...
                _logger.Debug($"_firstPacketLength = {_firstPacketLength}");
            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    lock (_decryptionLock)
                    {
                        ...
                        {
                            _logger.Error("decryption error");
                            Close();
                            return;
                        }
                    }
                    if (bytesToSend == 0)
                    {
                        ...
                        _logger.Debug("Need more to decrypt");
                        session.Remote.BeginReceive(_remoteRecvBuffer, 0, RecvSize, SocketFlags.None,
                            PipeRemoteReceiveCallback, session);
                        return;
                    }
                    _logger.Debug($"start sending {bytesToSend}");
        ...
        private void SendToServer(int length, AsyncSession session)
        {
            ...
            lock (_encryptionLock)
            {
                ...
                {
                    _logger.Debug("encryption error");
                    Close();
                    return;
                }
            }
        ...
        private void PipeRemoteSendCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (bytesRemaining > 0)
                {
                    _logger.Info("reconstruct _connetionSendBuffer to re-send");
        ...
        private void PipeConnectionSendCallback(IAsyncResult ar)
        {
            try
            {
                ...
                if (bytesRemaining > 0)
                {
                    _logger.Info("reconstruct _remoteSendBuffer to re-send");

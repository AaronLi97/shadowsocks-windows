...
using Shadowsocks.Std.Encryption.CircularBuffer;
using Shadowsocks.Std.Encryption.Exception;
using Shadowsocks.Std.Encryption.Stream;
...
namespace Shadowsocks.Std.Encryption.AEAD
{
        ...
        : EncryptorBase
    {
        ...
        private readonly ByteCircularBuffer _encCircularBuffer = new ByteCircularBuffer(MAX_INPUT_SIZE * 2);

        private readonly ByteCircularBuffer _decCircularBuffer = new ByteCircularBuffer(MAX_INPUT_SIZE * 2);
        ...
        public virtual void InitCipher(byte[] salt, bool isEncrypt, bool isUdp)
        {
            ...
            _logger.Dump("Salt", salt, saltLen);
        }
        ...
        public override void Encrypt(byte[] buf, int length, byte[] outbuf, out int outlength)
        {
            ...
            outlength = 0;
            _logger.Debug("---Start Encryption");
            if (!_encryptSaltSent)
            {
                ...
                outlength = saltLen;
                _logger.Debug($"_encryptSaltSent outlength {outlength}");
            }
            ...
            if (!_tcpRequestSent)
            {
                ...
                ChunkEncrypt(addrBytes, AddrBufLength, encAddrBufBytes, out int encAddrBufLength);
                ...
                _logger.Debug($"_tcpRequestSent outlength {outlength}");
            }
            ...
            while (true)
            {
                ...
                ChunkEncrypt(chunkBytes, chunklength, encChunkBytes, out int encChunkLength);
        ...
        public override void Decrypt(byte[] buf, int length, byte[] outbuf, out int outlength)
        {
            ...
            _logger.Debug("---Start Decryption");
            if (!_decryptSaltReceived)
            {
                ...
                _logger.Debug("get salt len " + saltLen);
            }
            ...
            while (true)
            {
                ...
                if (bufSize <= 0)
                {
                    _logger.Debug("No data in _decCircularBuffer");
                    return;
                }
                ...
                if (chunkLen > CHUNK_LEN_MASK)
                {
                    ...
                    _logger.Error($"Invalid chunk length: {chunkLen}");
                    throw new CryptoErrorException();
                }
                _logger.Debug("Get the real chunk len:" + chunkLen);
                bufSize = _decCircularBuffer.Size;
                if (bufSize < CHUNK_LEN_BYTES + tagLen /* we haven't remove them */+ chunkLen + tagLen)
                {
                    _logger.Debug("No more data to decrypt one chunk");
                    return;
                }
                ...
                _logger.Debug("aead dec outlength " + outlength);
                if (outlength + 100 > TCPHandler.BufferSize)
                {
                    _logger.Debug("dec outbuf almost full, giving up");
                    return;
                }
                bufSize = _decCircularBuffer.Size;
                ...
                if (bufSize <= 0)
                {
                    _logger.Debug("No data in _decCircularBuffer, already all done");
                    return;
                }
            }
        }
        ...
        private void ChunkEncrypt(byte[] plaintext, int plainLen, byte[] ciphertext, out int cipherLen)
        {
            ...
            {
                _logger.Error("enc chunk too big");
                throw new CryptoErrorException();
            }
